...
def retrive_posts_with_equal_content_title(request):
    ...
    """solution2"""
    posts = Post.objects.filter(view_count__gte=F('like_count') +10)
...
def add_template(request):
    now = datetime.now()
    PostTemplate.objects.create(title_template='some other template' , content='some other content' )
    new_template = PostTemplate.object.all()
