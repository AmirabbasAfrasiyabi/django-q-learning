...
def retrive_posts_exclude_sample(request):
    ...
    posts = posts.filter(created_date__range =(datetime.datetime.now()) - datetime.timedelta(days=1))
...
def get_comments(request):
    ...
    comments = Comment.objects.filter(post__title__contains='post', created_date__range=(now - datetime.timedelta(hours=1), now))
